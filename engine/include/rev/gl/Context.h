...
public:
  ResourceContext(GLuint resourceId)
      : _resourceId(resourceId), _previousContext(sCurrentContext) {
  ...
  ResourceContext(ResourceContext &&src)
      : _resourceId(src._resourceId), _previousContext(src._previousContext) {
    sCurrentContext = this;
    src._resourceId = 0;
  ...
  GLuint getResourceId() const { return _resourceId; }
