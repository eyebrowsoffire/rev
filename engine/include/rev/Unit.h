...
struct UnitComponentEnumeration {
...
struct CompositionMultiplier {
    ...
    using CombinedNumerator = SumIntegerSeq<typename FirstComposition::Numerator,
        typename SecondComposition::Numerator>;
    using CombinedDenominator = SumIntegerSeq<typename FirstComposition::Denominator,
        typename SecondComposition::Denominator>;

    using CanceledComponents = MinIntegerSeq<CombinedNumerator, CombinedDenominator>;
...
template <typename ValueType, typename Composition>
class Unit {
public:
    Unit()
        : _value(0)
    ...
    {
        using NewCompositionType = MultiplyComposition<Composition, OtherComposition>;
