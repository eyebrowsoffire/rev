...
struct IntegerGetter<std::index_sequence<integers...>> {
    ...
    struct GetAtIndex {
        static constexpr size_t value = GetAtIndex<index - 1, remainingIntegers...>::value;
    };
...
struct ZipIntegerSeq {
    ...
    struct Zip<std::integer_sequence<size_t, indexes...>> {
        using Type = std::integer_sequence<
            size_t, (Function<FirstGetter::template Get<indexes>, SecondGetter::template Get<indexes>>::value)...>;
    };
...
template <size_t first, size_t second>
struct SubtractIntegerFunction {
