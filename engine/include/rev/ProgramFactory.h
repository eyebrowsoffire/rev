...
// class Program {
//   public:
//     // Programs must have a constructor that takes a ProgramResource (compiled shader program)
//     // The ProgramFactory will use this to construct the object.
//     Program(ProgramResource programResource);
//
//     // Programs must declare a meta-structure called "Source" which provides the shader source
//     // code. The ProgramFactory compiles the source code on behalf of the program.
//     struct Source {
...
// ProgramFactory compiles and instantiates program objects from their source code, or returns an
// existing program object if it is already in use. This allows decoupled systems that use the
// same shader program to not have to compile the shader source multiple times.
class ProgramFactory
{
  public:
    // Compiles and instantiates the specified program type, or returns an existing instance of the
    // program if it has already been compiled.
